...
# see: https://github.com/hbar-boi/UFSDPyDAQ/blob/main/modules/stage.py
...
class CustomUnits(LittleEndianStructure):
    _pack_ = 1
    _fields_ = [("A", c_double), ("MicrostepMode", c_uint)]
...
class Position(Structure):
    _fields_ = [("Position", c_float), ("EncPosition", c_longlong)]
...
class Axis:
    ...
    def to(self, pos, wait=True):
        check(API.command_move_calb(self.axis, c_float(pos), byref(self.stage.units)))
    ...
    def getPosition(self):
        position = Position()
        check(
            API.get_position_calb(self.axis, byref(position), byref(self.stage.units))
        )
