...
LECROY_WAVEFORM_TEMPLATE = {
    "DESCRIPTOR_NAME": ["16s", 0, 16],
    "TEMPLATE_NAME": ["16s", 16, 32],
    "COMM_TYPE": ["h", 32, 34],
    "COMM_ORDER": ["h", 34, 36],
...
def trcReader(
    ...
    if sub_data == "WAV_DATA":
        if sequence == None:
            ...
            wave_count_bytes = binary_waveform[
                OFF_SET
                + LECROY_WAVEFORM_TEMPLATE["WAVE_ARRAY_COUNT"][1] : OFF_SET
                + LECROY_WAVEFORM_TEMPLATE["WAVE_ARRAY_COUNT"][2]
            ]
            ...
            vertical_offset_bytes = binary_waveform[
                OFF_SET
                + LECROY_WAVEFORM_TEMPLATE["VERTICAL_OFFSET"][1] : OFF_SET
                + LECROY_WAVEFORM_TEMPLATE["VERTICAL_OFFSET"][2]
            ]
            vertical_offset = struct.unpack(
                endianess + LECROY_WAVEFORM_TEMPLATE["VERTICAL_OFFSET"][0],
            ...
            vertical_gain_bytes = binary_waveform[
                ...
                + LECROY_WAVEFORM_TEMPLATE["VERTICAL_GAIN"][2]
            ]
            vertical_gain = struct.unpack(
                endianess + LECROY_WAVEFORM_TEMPLATE["VERTICAL_GAIN"][0],
                vertical_gain_bytes,
            )
            ...
            segment_count_bytes = binary_waveform[
                OFF_SET
                + LECROY_WAVEFORM_TEMPLATE["NOM_SUBARRAY_COUNT"][1] : OFF_SET
                + LECROY_WAVEFORM_TEMPLATE["NOM_SUBARRAY_COUNT"][2]
            ]
            ...
            trigger_time_array = binary_waveform[
                OFF_SET
                + LECROY_WAVEFORM_TEMPLATE["WAVE_SOURCE"][2]
                + USER_TEXT_bytes : OFF_SET
                + LECROY_WAVEFORM_TEMPLATE["WAVE_SOURCE"][2]
                + USER_TEXT_bytes
            ...
            for k in range(segment_count * 2):
                if (k + 1) % 2 != 0:
                    for i in range(wave_count - 2):
                        voltage_bit = wave_data_stream[
                            i * wave_data_byte_size
                            + k * wave_count : i * wave_data_byte_size
                            + wave_data_byte_size
                            + k * wave_count
                        ]
    ...
    if sub_data == "Trigger_Tdiff" and sequence != None:
        ...
        trigger_Tdiff_data = binary_waveform[
            OFF_SET
            + LECROY_WAVEFORM_TEMPLATE["WAVE_SOURCE"][2]
            + USER_TEXT_bytes : OFF_SET
            + LECROY_WAVEFORM_TEMPLATE["WAVE_SOURCE"][2]
        ...
        for i in range(numEvent):
            d = struct.unpack(
                endianess + "d",
                trigger_Tdiff_data[0 + i * d_byteSize : 8 + i * d_byteSize],
            )
            if i % 2 == 0:
    ...
    # base case
    data_bytes = binary_waveform[
        OFF_SET
        + LECROY_WAVEFORM_TEMPLATE[sub_data][1] : OFF_SET
        + LECROY_WAVEFORM_TEMPLATE[sub_data][2]
    ]
